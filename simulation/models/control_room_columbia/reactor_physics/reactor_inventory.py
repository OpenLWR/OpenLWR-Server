...
def run():
	...
	water_temperature = reactor_physics.reactor_water_temperature
	waterMass = reactor_physics.waterMass
	...
	if water_temperature > boilingPoint and waterMass>0:
		...
		reactor_physics.kgSteam+=vapMass["vm"]
		
		NewPress = pressure.getPressure(reactor_physics.kgSteam, reactor_physics.reactor_water_temperature,pressure.Volumes["Vessel"])
		pressure.Pressures["Vessel"]= NewPress
		waterMass -= vapMass["vm"]
		print(vapMass["vm"])
	else:
		NewPress = pressure.getPressure(reactor_physics.kgSteam, reactor_physics.reactor_water_temperature,pressure.Volumes["Vessel"])
		pressure.Pressures["Vessel"] = NewPress
